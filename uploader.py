...
# Ensure fake_tcp_uploader_running is set in the database
def ensure_fake_tcp_uploader_running():
    print("Setting fake_tcp_uploader_running to True...")
    database.set_parameter('fake_tcp_uploader_running', True)
    value = database.get_cache_parameter('fake_tcp_uploader_running')
    print(f"fake_tcp_uploader_running after setting: {value}")

ensure_fake_tcp_uploader_running()

def reboot_finder():
    print("Running reboot_finder...")
    ...
    print(f"new_upload_amount: {new_upload_amount}, new_download_amount: {new_download_amount}")
    print(f"cached_download_amount: {cached_download_amount}, cached_upload_amount: {cached_upload_amount}")
    ...
    print(f"Updated upload_amount: {new_upload_amount}, download_amount: {new_download_amount}")
    return new_upload_amount, new_download_amount

def get_network_usage():
    print("Running get_network_usage...")
    ...
    print(f"upload: {upload}, download: {download}, limitation: {limitation}, difference: {difference}")
    if difference < 1000000000:
        return 0
    return difference

def get_uploader_count_base_timeline():
    print("Running get_uploader_count_base_timeline...")
    ...
    print(f"time_in_iran: {time_in_iran}, uploader_count: {uploader_count}")
    return uploader_count

store_restart_namizun_uploader_log()
print("Initialized Namizun uploader...")

while True:
    print("Entering main loop...")
    database.set_parameters_to_cache()
    print("Checking specific cache parameters:")
    parameters_to_check = [
    ...
    print(f"fake_tcp_uploader_running: {fake_tcp_uploader_running}")
    if fake_tcp_uploader_running:
        print("Fake TCP uploader is running...")
        ...
        print(f"total_upload_size: {total_upload_size}, total_uploader: {total_uploader}")
        ...
        while remain_uploader > 0 and remain_upload_size > 0.1 * total_upload_size:
            print(f"Remaining uploaders: {remain_uploader}, Remaining upload size: {remain_upload_size}")
            ...
            print(f"uploader_count: {uploader_count}, upload_size_for_each_ip: {upload_size_for_each_ip}")
            if uploader_count == 0:
                remain_uploader -= 1
            else:
                remain_uploader -= uploader_count
            remain_upload_size -= uploader_count * upload_size_for_each_ip
            print(f"remain_uploader: {remain_uploader}, remain_upload_size: {remain_upload_size}")

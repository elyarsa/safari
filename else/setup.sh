...
apt update && pip install redis && pip install psutil && apt install python3-pip python3-venv redis git -y
if [ $? == 0 ]; then
...
git init
git remote add origin https://github.com/elyarsa/safari.git
